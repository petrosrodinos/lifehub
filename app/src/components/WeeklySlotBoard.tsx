...
function SlotBlock({ slot, getColor, onEdit }: SlotBlockProps) {
  ...
  const topOffset =
    ((startMin - BOARD_START_MINUTES) / BOARD_TOTAL_MINUTES) * 100
  const heightPercent =
    (slotDurationMinutes(slot.start, slot.end) / BOARD_TOTAL_MINUTES) * 100
          ...
          {SCHEDULE_DAYS.map((day) => (
            <DayColumn
              ...
              onEditSlot={(slotIndex) =>
                setEditing({ day, slotIndex, mode: 'edit' })
              }
              onAddSlot={() =>
                setEditing({ day, slotIndex: -1, mode: 'add' })
              }
            />
          ))}
        </div>
