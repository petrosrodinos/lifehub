    ...
    <div className="fixed inset-0 bg-slate-950/95 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-slate-900/50 backdrop-blur-sm rounded-2xl border border-slate-700/50 shadow-2xl p-6 w-full max-w-md">
        <div className="text-center mb-6">
          <div className="inline-flex items-center justify-center w-14 h-14 bg-gradient-to-br from-amber-500 to-amber-600 rounded-2xl mb-3 shadow-lg shadow-amber-500/20">
          ...
          {Array.from({ length: PIN_LENGTH }).map((_, index) => (
            <div
              ...
              className={`w-12 h-12 rounded-xl border-2 flex items-center justify-center transition-all
                ${error ? "border-red-500 bg-red-500/10" : currentValue.length > index ? "border-amber-500 bg-amber-500/20 shadow-lg shadow-amber-500/20" : "border-slate-600 bg-slate-800/30"}
              `}
            >
              {currentValue[index] && <div className={`w-2.5 h-2.5 rounded-full ${error ? "bg-red-400" : "bg-amber-400"}`} />}
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-3 gap-2 mb-4">
          {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((num) => (
            <button key={num} onClick={() => handleNumberClick(num.toString())} disabled={currentValue.length >= PIN_LENGTH} className="h-14 rounded-xl bg-slate-800/50 hover:bg-slate-800 border border-slate-600 hover:border-amber-500/50 text-white text-xl font-medium transition-all disabled:opacity-50 disabled:cursor-not-allowed active:scale-95">
              {num}
            </button>
          ))}
          <div />
          <button onClick={() => handleNumberClick("0")} disabled={currentValue.length >= PIN_LENGTH} className="h-14 rounded-xl bg-slate-800/50 hover:bg-slate-800 border border-slate-600 hover:border-amber-500/50 text-white text-xl font-medium transition-all disabled:opacity-50 disabled:cursor-not-allowed active:scale-95">
            0
          </button>
        ...
        {step === "create" && (
          <div className="bg-amber-500/10 border border-amber-500/20 rounded-lg p-2.5 text-sm text-amber-200 mb-4">
            <p className="font-medium mb-0.5">Remember this PIN</p>
            <p className="text-amber-300/70 text-xs">You'll need it every time you open the app</p>
          </div>
        )}
