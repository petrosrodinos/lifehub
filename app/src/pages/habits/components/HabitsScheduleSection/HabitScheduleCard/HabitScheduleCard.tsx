  ...
  return (
    <section className="rounded-2xl border border-slate-700/60 bg-slate-900/60 p-4 sm:p-5 space-y-4">
      <header className="flex items-center justify-between">
        ...
        <button type="button" onClick={() => setIsEditing((value) => !value)} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs border border-slate-600 text-slate-200 hover:bg-slate-800 transition-colors">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm">
        ...
        <div className="rounded-xl border border-slate-700/50 bg-slate-800/50 p-3">
          ...
          <p className="text-slate-100">{activeSchedule.target_type === "BOOLEAN" ? "Mark as done" : `${activeSchedule.target_value ?? 0} ${activeSchedule.target_unit_label ?? activeSchedule.target_unit?.toLowerCase() ?? "units"}`}</p>
        </div>
        <div className="rounded-xl border border-slate-700/50 bg-slate-800/50 p-3">
        ...
        <div className="rounded-xl border border-amber-500/30 bg-amber-500/5 p-3 space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <label className="space-y-1.5">
              ...
              <select value={repeatType} onChange={(event) => setRepeatType(event.target.value as ActivityRepeatType)} className="w-full px-3 py-2 rounded-lg bg-slate-900 border border-slate-700 text-slate-100 focus:outline-none focus:border-violet-500">
            ...
            <label className="space-y-1.5">
              ...
              <select value={targetType} onChange={(event) => setTargetType(event.target.value as "BOOLEAN" | "QUANTITY")} className="w-full px-3 py-2 rounded-lg bg-slate-900 border border-slate-700 text-slate-100 focus:outline-none focus:border-violet-500">
            ...
            <label className="space-y-1.5">
              ...
              <input type="number" min="0" value={targetValue} onChange={(event) => setTargetValue(event.target.value)} className="w-full px-3 py-2 rounded-lg bg-slate-900 border border-slate-700 text-slate-100 focus:outline-none focus:border-violet-500" />
            </label>
            ...
            <label className="space-y-1.5">
              ...
              <input type="time" value={timeOfDay} onChange={(event) => setTimeOfDay(event.target.value)} className="w-full px-3 py-2 rounded-lg bg-slate-900 border border-slate-700 text-slate-100 focus:outline-none focus:border-violet-500" />
            </label>
          </div>

          <button type="button" disabled={isSaving} onClick={handleSave} className="inline-flex items-center gap-2 px-3.5 py-2 rounded-lg bg-violet-600 hover:bg-violet-500 text-white text-sm disabled:opacity-60 transition-colors">
