...
function getStatusIcon(status: ActivityTodayItem['status']) {
  ...
  if (status === 'SKIPPED') return <MinusCircle className="w-4 h-4" />
  if (status === 'FAILED') return <CircleX className="w-4 h-4" />
...
function formatTargetSummary(item: ActivityTodayItem) {
  ...
  const value = item.quantityValue ?? 0
  ...
  return (
    <button
      ...
      className={`w-full text-left rounded-2xl border transition-all duration-200 ${
        isActive
          ? 'border-violet-400/70 bg-slate-800/90 shadow-[0_14px_34px_rgba(139,92,246,0.22)]'
          : 'border-slate-700/60 bg-slate-900/60 hover:border-slate-500/70 hover:bg-slate-800/60'
      ...
      <div className="p-4 sm:p-5 space-y-4">
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-start gap-3 min-w-0">
            <div
              className="w-10 h-10 rounded-xl grid place-items-center text-sm font-semibold border border-white/10"
              style={{ backgroundColor: `${item.activity.color ?? '#8b5cf6'}30`, color: item.activity.color ?? '#c4b5fd' }}
            >
