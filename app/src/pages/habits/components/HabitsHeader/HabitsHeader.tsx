...
export function HabitsHeader() {
  const { dateLabel, completedToday, totalToday } = useHabitsHeader();
  ...
  return (
    <>
      <header className="rounded-2xl border border-slate-700/60 bg-slate-900/65 backdrop-blur-sm px-4 sm:px-5 py-4">
        ...
        <div className="mt-2 flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            <button type="button" onClick={() => setIsCreateScheduleOpen(true)} className="inline-flex items-center gap-1.5 px-2.5 xs:px-3 py-1.5 xs:py-2 rounded-xl border border-emerald-500/35 bg-emerald-500/10 text-emerald-200 text-xs xs:text-sm hover:bg-emerald-500/20 transition-colors">
      ...
      <ActivityScheduleForm isOpen={isCreateScheduleOpen} onClose={() => setIsCreateScheduleOpen(false)} />
    </>
  );
}
