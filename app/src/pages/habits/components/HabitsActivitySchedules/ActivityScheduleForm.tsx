...
import {
  ...
  type ActivityTargetUnit,
  type CreateActivityScheduleDto,
...
type ActivityScheduleFormProps = {
  isOpen: boolean
  onClose: () => void
  defaultActivityUuid?: string
}

const INPUT_CLASS =
  'w-full px-4 py-3 bg-slate-800 border border-slate-700 rounded-xl text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-violet-500/50 focus:border-transparent transition-all'

export function ActivityScheduleForm({ isOpen, onClose, defaultActivityUuid }: ActivityScheduleFormProps) {
  const createSchedule = useCreateActivitySchedule()
  const activities = useActivities().data ?? []

  const [activityUuid, setActivityUuid] = useState(defaultActivityUuid ?? '')
  ...
  const isFormValid =
    activityUuid.length > 0 &&
    (repeatType !== ActivityRepeatTypes.WEEKDAYS || weekdays.length > 0) &&
    (repeatType !== ActivityRepeatTypes.INTERVAL || Number(intervalDays) > 0) &&
    (repeatType !== ActivityRepeatTypes.FREQUENCY || Number(frequencyValue) > 0) &&
    (targetType !== ActivityTargetTypes.QUANTITY || Number(targetValue) > 0)
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const data: CreateActivityScheduleDto = { repeat_type: repeatType, target_type: targetType }
    ...
    createSchedule.mutate({ activity_uuid: activityUuid, data }, { onSuccess: onClose })
  }
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        {repeatType === ActivityRepeatTypes.FREQUENCY && (
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-2">
              <label htmlFor="frequencyValue" className="block text-sm font-semibold text-slate-300">Times</label>
            ...
            <div className="space-y-2">
              <label htmlFor="frequencyPeriod" className="block text-sm font-semibold text-slate-300">Period</label>
        ...
        {targetType === ActivityTargetTypes.QUANTITY && (
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-2">
              <label htmlFor="targetValue" className="block text-sm font-semibold text-slate-300">Target Value</label>
            ...
            <div className="space-y-2">
              <label htmlFor="targetUnit" className="block text-sm font-semibold text-slate-300">Unit</label>
