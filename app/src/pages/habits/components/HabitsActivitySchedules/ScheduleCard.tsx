...
import { useScheduleCard } from "./hooks/use-schedule-card";
...
export function ScheduleCard({ schedule, onSelect }: ScheduleCardProps) {
  const {
    ...
    confirmDelete,
    isDeleting,
  } = useScheduleCard(schedule);

  const repeatDetail = formatRepeatDetail(schedule);

  const handleSelect = () => {
    onSelect?.(schedule.uuid);
  };
    ...
    <>
      <div className={`rounded-xl border p-3 sm:p-4 transition-colors ${schedule.is_active ? "border-violet-500/40 bg-violet-500/5" : "border-slate-700/50 bg-slate-800/40"}`}>
        <div className="flex items-center justify-between gap-2">
          <button type="button" onClick={handleSelect} className="flex items-center gap-2 min-w-0 flex-1 text-left">
            {schedule.is_active ? (
        ...
        <button type="button" onClick={handleSelect} className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-x-4 sm:gap-y-2 w-full text-left">
        ...
        {isEditOpen && <ScheduleEditForm schedule={schedule} onSave={saveSchedule} onCancel={closeEdit} isPending={isUpdating} />}
      </div>
