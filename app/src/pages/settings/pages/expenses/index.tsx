...
import {
  useHiddenCategories,
  useCreateHiddenCategory,
  useDeleteHiddenCategory,
} from "../../../../features/expenses/hidden-categories/hooks/use-hidden-categories";
import {
  useHiddenSubcategories,
  useCreateHiddenSubcategory,
  useDeleteHiddenSubcategory,
} from "../../../../features/expenses/hidden-subcategories/hooks/use-hidden-subcategories";
...
export function ExpensesSettingsPage() {
  ...
  const categoriesNotHidden = useMemo(
    () => categories.filter((c) => !hiddenCategoryUuids.has(c.uuid)),
    [categories, hiddenCategoryUuids]
  );
  const subcategoriesNotHidden = useMemo(
    () => subcategories.filter((s) => !hiddenSubcategoryUuids.has(s.uuid)),
    [subcategories, hiddenSubcategoryUuids]
  );
  ...
  const handleHideCategory = () => {
    ...
    createHiddenCategory.mutate(
      { category_uuid: selectedCategoryUuid },
      { onSuccess: () => setSelectedCategoryUuid("") }
    );
  };

  const handleHideSubcategory = () => {
    if (!selectedSubcategoryUuid) return;
    createHiddenSubcategory.mutate(
      { subcategory_uuid: selectedSubcategoryUuid },
      { onSuccess: () => setSelectedSubcategoryUuid("") }
    );
  };
            ...
            <section>
              ...
              <div className="bg-slate-900/50 backdrop-blur-sm rounded-xl border border-slate-700/50 p-6 space-y-4">
                    ...
                    {hiddenCategories.map((h) => (
                      ...
                      >
                        <span className="text-white">
                          {categoryByUuid.get(h.category_uuid)?.name ?? h.category_uuid}
                        </span>
                        <button
                          type="button"
                          onClick={() => deleteHiddenCategory.mutate(h.uuid)}
                          disabled={deleteHiddenCategory.isPending}
                          className="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm bg-slate-700/50 hover:bg-slate-600/50 text-slate-300 rounded-lg transition-colors"
                        >
                ...
                {categoriesNotHidden.length > 0 && (
                  <div className="pt-4 border-t border-slate-700/50 flex flex-wrap items-end gap-2">
                    <div className="flex-1 min-w-[200px]">
                      ...
                      <select
                        ...
                        className="w-full px-3 py-2 bg-slate-800 border border-slate-600 rounded-lg text-white focus:ring-2 focus:ring-violet-500 focus:border-transparent"
                      >
                    ...
                    <button
                      ...
                      disabled={!selectedCategoryUuid || createHiddenCategory.isPending}
                      className="inline-flex items-center gap-2 px-4 py-2 bg-violet-500 hover:bg-violet-600 disabled:opacity-50 text-white rounded-lg font-medium transition-colors"
                    >
                ...
                {hiddenSubcategories.length === 0 ? (
                  ...
                  <ul className="space-y-2">
                    {hiddenSubcategories.map((h) => (
                      ...
                      >
                        <span className="text-white">
                          {subcategoryByUuid.get(h.subcategory_uuid)?.name ?? h.subcategory_uuid}
                        </span>
                        <button
                          type="button"
                          onClick={() => deleteHiddenSubcategory.mutate(h.uuid)}
                          disabled={deleteHiddenSubcategory.isPending}
                          className="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm bg-slate-700/50 hover:bg-slate-600/50 text-slate-300 rounded-lg transition-colors"
                        >
                          <Eye className="w-4 h-4" />
                          Show
                  ...
                  <div className="pt-4 border-t border-slate-700/50 flex flex-wrap items-end gap-2">
                    <div className="flex-1 min-w-[200px]">
                      ...
                      <select
                        ...
                        onChange={(e) => setSelectedSubcategoryUuid(e.target.value)}
                        className="w-full px-3 py-2 bg-slate-800 border border-slate-600 rounded-lg text-white focus:ring-2 focus:ring-violet-500 focus:border-transparent"
                      >
                    ...
                    <button
                      type="button"
                      onClick={handleHideSubcategory}
                      disabled={!selectedSubcategoryUuid || createHiddenSubcategory.isPending}
                      className="inline-flex items-center gap-2 px-4 py-2 bg-violet-500 hover:bg-violet-600 disabled:opacity-50 text-white rounded-lg font-medium transition-colors"
                    >
                      <EyeOff className="w-4 h-4" />
                      Hide
