...
import {
  useHiddenActivities,
  useCreateHiddenActivity,
  useDeleteHiddenActivity,
} from "../../../../features/habbits/hidden-activities/hooks/use-hidden-activities";
...
export function ActivitiesSettingsPage() {
  ...
  const activitiesNotHidden = useMemo(
    () => activities.filter((a) => !hiddenActivityUuids.has(a.uuid)),
    [activities, hiddenActivityUuids]
  );
  ...
  const handleHideActivity = () => {
    ...
    createHiddenActivity.mutate(
      { activity_uuid: selectedActivityUuid },
      { onSuccess: () => setSelectedActivityUuid("") }
    );
  };
                    ...
                    {hiddenActivities.map((h) => (
                      ...
                      >
                        <span className="text-white">
                          {activityByUuid.get(h.activity_uuid)?.name ?? h.activity_uuid}
                        </span>
                        <button
                          type="button"
                          onClick={() => deleteHiddenActivity.mutate(h.uuid)}
                          disabled={deleteHiddenActivity.isPending}
                ...
                {activitiesNotHidden.length > 0 && (
                  <div className="pt-4 border-t border-slate-700/50 flex flex-wrap items-end gap-2">
                    ...
                    <button
                      ...
                      disabled={!selectedActivityUuid || createHiddenActivity.isPending}
