...
import type { ScheduleDay } from "../../../../features/routine/interfaces/routine.interface";
import { SCHEDULE_DAYS } from "../../../../features/routine/interfaces/routine.interface";
import { useDuplicateDay } from "../../../../features/routine/hooks/use-routine";

export type DuplicateDayModalProps = {
  sourceDay: ScheduleDay;
  onClose: () => void;
};

export function DuplicateDayModal({ sourceDay, onClose }: DuplicateDayModalProps) {
      ...
      <div className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md bg-slate-800 border border-slate-600 rounded-xl shadow-xl z-50 p-6" role="dialog" aria-label="Duplicate day">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-2 pt-2">
            ...
            <button type="submit" disabled={duplicateDay.isPending || selectedDays.length === 0} className="flex-1 px-4 py-2 bg-violet-500 hover:bg-violet-600 text-white font-medium rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
