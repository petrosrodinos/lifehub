...
function SlotBlock({ slot, onEdit }: SlotBlockProps) {
  ...
  const heightPercent =
    (slotDurationMinutes(slot.start_time, slot.end_time) / BOARD_TOTAL_MINUTES) * 100
...
function DayColumn({ day, slots, onEditSlot, onAddSlot }: DayColumnProps) {
  return (
    <div className="flex flex-col min-w-[140px] flex-1">
      <h3 className="text-sm font-semibold text-slate-300 uppercase tracking-widest mb-3 px-2 text-center">
        {day}
      </h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-3 py-2 bg-slate-900 border border-slate-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-amber-500/50 capitalize"
            >
              {activities.length === 0 ? (
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-slate-100 p-6 md:p-10 flex items-center justify-center">
        <div className="text-center max-w-md">
          ...
          <p className="text-slate-400 mb-6">
            Create your first activity to start building your weekly schedule
          </p>
          <button
            type="button"
            onClick={() => setShowActivitiesMenu(true)}
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="text-center max-w-md mx-auto py-12 mb-8">
            ...
            <p className="text-sm text-slate-500 mb-6">
              Click "+ Add slot" button in any day column to get started
            </p>
          </div>
          <div className="flex gap-4 overflow-x-auto pb-4">
            {SCHEDULE_DAYS.map((day) => (
              <DayColumn
                ...
                day={day}
                slots={[]}
                onEditSlot={(slot) => setEditing({ day, slot, mode: 'edit' })}
                onAddSlot={() => setEditing({ day, mode: 'add' })}
              />
            ))}
          </div>
          {editing && (
          ...
          <div className="mt-8 flex flex-wrap gap-3 justify-center">
            {activities.map((activity) => (
              <span
                key={activity.uuid}
                className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-medium text-white capitalize"
