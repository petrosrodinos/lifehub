  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <div className="space-y-2">
        ...
        <select id="upload-account" value={fromAccountUuid} onChange={(e) => setFromAccountUuid(e.target.value)} className="w-full px-4 py-3 bg-slate-800 border border-slate-700 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-violet-500/50 focus:border-transparent transition-all" disabled={isUploadPending} required>
      ...
      <ReceiptPhotoCapture onFileSelect={handleFileSelect} onFileClear={handleFileClear} disabled={isUploadPending} />

      <div className="flex gap-3 pt-2">
        <button type="submit" disabled={isUploadPending || !canSubmit} className="flex-1 px-4 py-3 bg-gradient-to-r from-violet-500 to-blue-500 hover:from-violet-600 hover:to-blue-600 text-white font-semibold rounded-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
          {isUploadPending && (
        ...
        <button type="button" onClick={onCancel} disabled={isUploadPending} className="px-6 py-3 bg-slate-800 hover:bg-slate-700 text-slate-200 font-semibold rounded-xl transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
          Cancel
        </button>
      </div>
    </form>
