...
type ReceiptFormProps = {
  ...
  onUploadSuccess?: (receipt: ExpenseReceipt) => void;
};

export function ReceiptForm({ onSubmit, onCancel, submitLabel, isPending = false, initialData, onUploadSuccess }: ReceiptFormProps) {
  ...
  const handleUploadSubmit = (file: File, fromAccountUuid: string): void => {
    uploadReceipt.mutate({ file, from_account_uuid: fromAccountUuid }, { onSuccess: (receipt: ExpenseReceipt) => onUploadSuccess?.(receipt) });
  };

  return (
    <div className="space-y-5">
      <ReceiptTabSwitcher activeTab={activeTab} onTabChange={setActiveTab} />

      {activeTab === RECEIPT_TABS.MANUAL && <ReceiptManualTab expenseEntryUuid={expenseEntryUuid} storeUuid={storeUuid} receiptDate={receiptDate} totalAmount={totalAmount} entries={entries} stores={stores} isPending={isPending} isEntryDisabled={!!initialData?.expense_entry_uuid} submitLabel={submitLabel} onExpenseEntryChange={setExpenseEntryUuid} onStoreChange={setStoreUuid} onDateChange={setReceiptDate} onAmountChange={setTotalAmount} onSubmit={handleManualSubmit} onCancel={onCancel} />}

      {activeTab === RECEIPT_TABS.UPLOAD && <ReceiptUploadTab accounts={accounts} isUploadPending={uploadReceipt.isPending} onSubmit={handleUploadSubmit} onCancel={onCancel} />}
    </div>
  );
}
