...
export function PriceEvolutionChart({ data }: PriceEvolutionChartProps) {
  if (data.length === 0) {
    return <div className="flex items-center justify-center h-64 text-slate-400">No price data available for the selected product</div>;
  }
  ...
  return (
    <ResponsiveContainer width="100%" height={350}>
      <LineChart data={formattedData} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
        ...
        <Line type="monotone" dataKey="unit_price" stroke="#8b5cf6" strokeWidth={2} dot={{ fill: "#8b5cf6", r: 4 }} activeDot={{ r: 6 }} name="Unit Price" />
      </LineChart>
    </ResponsiveContainer>
  );
}
