...
export function ReceiptItemCard({ item, receiptUuid }: ReceiptItemCardProps) {
  ...
  const handleUpdate = (data: { receipt_uuid: string; quantity?: number; unit_price: number; total_price: number; product_uuid?: string }) => {
    ...
    updateItem.mutate({ uuid: item.uuid, data: updateData }, { onSuccess: () => setIsEditOpen(false) });
  };
  ...
  return (
    <>
      <div className="py-2 px-2.5 sm:py-2.5 sm:px-3 bg-slate-800/30 rounded-lg hover:bg-slate-800/50 transition-colors group">
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <div className="hidden sm:flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
            <button type="button" onClick={() => setIsEditOpen(true)} className="p-1.5 text-slate-400 hover:text-violet-400 hover:bg-violet-500/10 rounded-lg transition-colors">
              <Pencil className="w-3.5 h-3.5" />
            </button>
            <button type="button" onClick={() => setIsDeleteOpen(true)} className="p-1.5 text-slate-400 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-colors">
      ...
      <ConfirmationModal isOpen={isDeleteOpen} onClose={() => setIsDeleteOpen(false)} onConfirm={handleDelete} title="Delete Item" description={`Are you sure you want to delete "${item.product?.name}"?`} confirmText="Delete" cancelText="Cancel" variant="danger" isPending={deleteItem.isPending} />
    </>
  );
}
