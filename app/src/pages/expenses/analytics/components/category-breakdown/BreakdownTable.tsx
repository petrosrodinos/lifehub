...
export function BreakdownTable({ data, groupBy }: BreakdownTableProps) {
  if (data.length === 0) {
    return null;
  }
    ...
    <div className="bg-slate-900/50 backdrop-blur-sm rounded-xl border border-slate-800/50 p-6">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-semibold text-white">
          Detailed Breakdown by {groupBy === "category" ? "Category" : "Subcategory"}
        </h3>
      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          ...
          <tbody>
            {data.map((item) => (
              <tr key={item.uuid} className="border-b border-slate-800/50 hover:bg-slate-800/30 transition-colors">
          ...
          <tfoot>
            <tr className="border-t-2 border-slate-700">
              <td colSpan={groupBy === "subcategory" ? 2 : 1} className="py-3 px-2 text-sm font-semibold text-white">
                Total
              </td>
