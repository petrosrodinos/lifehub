...
export function PurchaseHistoryTable({ purchases }: PurchaseHistoryTableProps) {
  ...
  return (
    <>
          ...
          <tbody>
            {sorted.map((p, index) => (
              <tr key={p.uuid} className="border-t border-slate-800/50">
                ...
                <td className="px-4 py-3">
                  <button type="button" onClick={() => setEditingPurchase(p)} className="p-1.5 text-slate-400 hover:text-violet-400 hover:bg-violet-500/10 rounded-md transition-colors" aria-label="Edit purchase">
      ...
      {editingPurchase && <EditPurchaseModal isOpen={!!editingPurchase} onClose={() => setEditingPurchase(null)} purchase={editingPurchase} />}
    </>
  );
}
